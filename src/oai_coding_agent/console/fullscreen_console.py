...
from prompt_toolkit.formatted_text import AnyFormattedText, FormattedText, ANSI
...
from oai_coding_agent.agent.events import MessageOutputEvent, ToolCallEvent, ReasoningEvent
from oai_coding_agent.console.rendering import render_message
...
class FullscreenConsole:
    ...
    def _create_key_bindings(self) -> KeyBindings:
        ...
        def _(event) -> None:
            """Navigate history popup if visible."""
            if self.fs_state.history_popup_visible:
                if self.fs_state.selected_history_index < len(self.fs_state.response_history) - 1:
                    self.fs_state.selected_history_index += 1
                    event.app.invalidate()
    ...
    def _append_to_scrollback(self, text: str) -> None:
        ...
        current = self.scrollback_buffer.text
        if current and not current.endswith('\n'):
            current += '\n'
        self.scrollback_buffer.document = Document(current + text + '\n')
        # Scroll to bottom
        self.scrollback_buffer.cursor_position = len(self.scrollback_buffer.text)
        if self.app:
            self.app.invalidate()
    ...
    def _render_rich_to_scrollback(self, renderable: Any) -> str:
        ...
        console = RichConsole(file=buf, width=self._get_terminal_width(), legacy_windows=False)
    ...
    def _update_engine_output(self, engine_id: str, content: str) -> None:
        ...
        lines = self.scrollback_buffer.text.split('\n')
        ...
        if engine.start_line < len(lines):
            ...
            # Update the buffer
            self.scrollback_buffer.document = Document('\n'.join(lines))
            self.scrollback_buffer.cursor_position = len(self.scrollback_buffer.text)
    ...
    def _get_footer_text(self) -> AnyFormattedText:
        ...
        if self.fs_state.interrupt_visible:
            if self.fs_state.interrupt_hide_time and asyncio.get_event_loop().time() > self.fs_state.interrupt_hide_time:
    ...
    def _create_layout(self) -> Container:
        ...
        # Wrap with float container for popups
        return FloatContainer(
            ...
            ] if self.fs_state.history_popup_visible else [],
        )
    ...
    def _create_history_popup(self) -> Container:
        ...
        for i, item in enumerate(self.fs_state.response_history[-10:]):
            ...
            lines.append(f"{prefix}{item.get('summary', 'Response')} - {item.get('created_at', '')}")
    ...
    async def _process_input(self, text: str) -> None:
        ...
        try:
            ...
            engine = EngineState(
                ...
                color="cyan",
                is_streaming=True,
                start_line=len(self.scrollback_buffer.text.split('\n')) - 1,
            )
                ...
                async for event in event_stream:
                    ...
                    if ui_msg:
                        ...
                        if isinstance(event, MessageOutputEvent):
                            ...
                            engine.tokens = len(message_content.split())  # Simple token estimate
                            self._update_engine_output(engine_id, message_content)
                        elif isinstance(event, ToolCallEvent):
                            # Add tool call info to content
                            tool_info = f"\n\n[dim]Tool: {event.tool_name}[/dim]"
                            self._update_engine_output(engine_id, engine.content + tool_info)
            ...
            try:
                ...
                self.fs_state.interrupt_hide_time = asyncio.get_event_loop().time() + 2.0
